<script setup>
  ...
  //护工：只显示并回填分配给自己的任务，隐藏删除入口
  const isHugong = computed(() => (userInfoStore.roleCodes || []).includes('hugong'))
  ...
  const loadOptions = () => {
    //护工不需要老人/护理项目下拉（任务列表已按本人过滤）
    if (isHugong.value) return
  ...
  <el-card class="">
    <template #header>
      <div class="page-header">
        <div class="page-title">护理任务管理</div>
        <div class="header">
          <el-button v-if="!isHugong" type="danger" :icon="Delete" @click="deleteAll" :disabled="!hasBtnPermission('careTask:deleteAll')">批量删除</el-button>
        </div>
      </div>
    </template>
    <el-alert
        v-if="isHugong"
        title="您仅可查看并回填分配给自己的护理任务，不能修改他人记录或进行删除。"
        type="info"
        :closable="false"
        style="margin-bottom: 14px"
    />
    <el-form :inline="true">
      <el-form-item v-if="!isHugong" label="老人">
      ...
      <el-form-item v-if="!isHugong" label="护理项目">
    ...
    <el-table :data="list" border style="width: 100%" show-overflow-tooltip ref="multipleTableRef" @selection-change="handleSelectionChange">
      ...
      <el-table-column align="center" width="200px" fixed="right" label="操作">
        <template #default="{ row }">
          ...
          <el-button v-if="!isHugong" size="small" type="danger" @click="deleteById(row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination
