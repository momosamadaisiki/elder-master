<script setup>
  ...
  const showAddDialog = () => {
    ...
    title.value = '添加'
    careLevel.value = {status: 1, sort: 0}
  }
  ...
  <el-card class="">
    <template #header>
      <div class="page-header">
        <div class="page-title">护理等级管理</div>
        <div class="header">
          <el-button type="primary" :icon="Plus" @click="showAddDialog" :disabled="!hasBtnPermission('careLevel:add')">添加</el-button>
          <el-button type="danger" :icon="Delete" @click="deleteAll" :disabled="!hasBtnPermission('careLevel:deleteAll')">批量删除</el-button>
        </div>
      </div>
    </template>
    <el-form :inline="true">
