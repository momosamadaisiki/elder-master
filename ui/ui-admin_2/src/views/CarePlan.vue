<script setup>
  ...
  const showAddDialog = () => {
    ...
    title.value = '添加'
    carePlan.value = {status: 1, carePlanItemList: []}
  }
  ...
  const addOrUpdate = () => {
    ...
    if (carePlan.value.startDate && carePlan.value.endDate && carePlan.value.startDate > carePlan.value.endDate) {
  ...
  <el-card class="">
    <template #header>
      <div class="page-header">
        <div class="page-title">护理计划管理</div>
        <div class="header">
          <el-button type="primary" :icon="Plus" @click="showAddDialog" :disabled="!hasBtnPermission('carePlan:add')">添加</el-button>
          <el-button type="danger" :icon="Delete" @click="deleteAll" :disabled="!hasBtnPermission('carePlan:deleteAll')">批量删除</el-button>
        </div>
      </div>
    </template>
    <el-form :inline="true">
